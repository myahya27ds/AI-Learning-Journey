...
def square_root():
    ...
    if a < 0:
        print("❌ Cannot calculate sqrt of negative number!")
        return
    result = math.sqrt(a)
    calculation = f"√{format_number(a)} = {format_decimal(result)}"
    print(calculation)
    save_history(calculation)
    print("="*40)
...
def trig_function(func, name):
    ...
    if func == math.tan and math.isclose(math.cos(a), 0, abs_tol=1e-9):
        print("❌ Tan undefined for this angle!")
        return
...
menu = {
    ...
    "4": ("Multiplication", lambda: binary_operation("×", operator.mul, "Multiplication")),
